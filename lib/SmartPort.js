...
class SmartPort {
    ...
    _buildHeaders(token, contentType) {
        ...
        if (tk) {
            const basicValue = String(tk).includes(':') ? String(tk) : `${tk}:`;
            headers['Authorization'] = 'Basic ' + Buffer.from(basicValue).toString('base64');
        }
        return headers;
    }

    _appendFormFields(container, value, prefix) {
        if (value === undefined || value === null) return;

        if (Array.isArray(value)) {
            value.forEach((item, index) => {
                this._appendFormFields(container, item, `${prefix}[${index}]`);
            });
            return;
        }
        ...
        if (isPlainObject) {
            for (const [childKey, childValue] of Object.entries(value)) {
                this._appendFormFields(container, childValue, `${prefix}[${childKey}]`);
            }
            return;
        }

        const normalizedValue = value === true ? '1' : value === false ? '0' : String(value);
        container.append(prefix, normalizedValue);
    }

    async fetchFromAPI(endpoint, params = {}, { token, method = 'GET', body, files, formEncoding } = {}) {
        try {
            ...
            if (method === 'GET') {
                ...
                if (formEncoding === 'php') {
                    for (const [key, val] of Object.entries(data)) {
                        this._appendFormFields(form, val, key);
                    }
                } else {
                    for (const [key, val] of Object.entries(data)) {
                ...
                const tk = token || this.token;
                const basicValue = tk ? (String(tk).includes(':') ? String(tk) : `${tk}:`) : null;
                const authHeaders = basicValue ? { 'Authorization': 'Basic ' + Buffer.from(basicValue).toString('base64') } : {};
                const multipartHeaders = { ...form.getHeaders(), ...authHeaders };

                // Algunos backends PHP/CodeIgniter requieren Content-Length explícito
                // para hidratar correctamente $_FILES.
                try {
                    ...
                    if (Number.isFinite(contentLength) && contentLength >= 0) {
                        multipartHeaders['Content-Length'] = String(contentLength);
                    }
                } catch (err) {
                    // Si no se puede calcular, mantener comportamiento previo sin romper uploads.
                }
                ...
                if (formEncoding === 'php') {
                    for (const [key, val] of Object.entries(data)) {
                        this._appendFormFields(formData, val, key);
                    }
                } else {
                    for (const [key, val] of Object.entries(data)) {
                        if (val !== undefined && val !== null) {
    ...
    async postToAPI(endpoint, body = {}, { token, files, formEncoding } = {}) {
        return this.fetchFromAPI(endpoint, {}, { token, method: 'POST', body, files, formEncoding });
    }
    ...
    async updateCache(endpoint, params = {}, { token } = {}) {
    ...
    async refresh(endpoint, params = {}, { token } = {}) {
        await this.updateCache(endpoint, params, { token });
    }
    ...
    async getData(endpoint, { ev = "undefined", limit, skip = 0, sort, filter, search, token } = {}) {
                    ...
                    result = result.filter(item =>
                        filter.$or.some(cond =>
                            Object.entries(cond).every(([key, val]) => {
                                ...
                                if (fieldVal && typeof fieldVal === 'object' && '$date' in fieldVal)
    ...
    async postAPI(endpoint, body = {}, { ev, token, files, formEncoding } = {}) {
        if (!body.ev) body.ev = ev || this.defaultEv;
        const data = await this.postToAPI(endpoint, body, { token, files, formEncoding });
        if (!data) return [];
        return data.answer ?? data;
    }
    ...
    getRouter() {
        ...
        // Acepta endpoint=a, endpoint[]=a y arreglos ya parseados por Express.
        const normalizeEndpoints = (query) => {
            const raw = query.endpoint ?? query['endpoint[]'];
            if (!raw) return [];
        ...
        const handleDeleteMultiple = (req, res) => {
            ...
            const endpoints = normalizeEndpoints(queryParams);
        ...
        const handleUpdateMultiple = async (req, res) => {
            ...
            const endpoints = normalizeEndpoints(originalParams);
            if (!endpoints.length) {
                return res.status(400).json({ success: false, error: 'Se esperaba endpoint[]=alias1&endpoint[]=alias2' });
            }
        ...
        const handleData = async (req, res, alias) => {
            let { ev, limit, skip, sort, search, token, ...rest } = req.query;
